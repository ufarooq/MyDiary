...
public class SettingActivity extends AppCompatActivity
    implements View.OnClickListener,
        SettingColorPickerFragment.colorPickerCallback,
...
//    outState.putString("tempFileManager", gson.toJson(tempFileManager));
    long end = Calendar.getInstance().getTimeInMillis();
    Log.i(TAG, "Save: " + getBundleSizeInBytes(outState) + ":" + (end - start));
  }
...
//        gson.fromJson(savedInstanceState.getString("isLanguageFirstRun"), boolean.class);
    ...
    isAddNewProfileBg =
        gson.fromJson(savedInstanceState.getString("isAddNewProfileBg"), boolean.class);
//    tempFileManager =
//        gson.fromJson(savedInstanceState.getString("tempFileManager"), FileManager.class);

    long end = Calendar.getInstance().getTimeInMillis();

    Log.i(TAG, "Restore: " + getBundleSizeInBytes(savedInstanceState) + ":" + (end - start));
  }
    ...
    if (requestCode == SELECT_PROFILE_BG) {
      if (resultCode == RESULT_OK) {
        if (data != null && data.getData() != null) {
          ...
          UCrop.of(
                  ...
                  Uri.fromFile(
                      new File(
                          tempFileManager.getDir() + "/" + FileManager.createRandomFileName())))
              .withMaxResultSize(bgWidth, bgHeight)
              .withOptions(options)
              .withAspectRatio(bgWidth, bgHeight)
              .start(this);
        } else {
          Toast.makeText(this, getString(R.string.toast_photo_intent_error), Toast.LENGTH_LONG)
              .show();
        }
      }
      ...
      if (resultCode == RESULT_OK) {
        if (data != null) {
          ...
          Toast.makeText(
                  this, getString(R.string.toast_crop_profile_banner_fail), Toast.LENGTH_LONG)
              .show();
          // sample error
  ...
  private void initSpinner() {
    ...
    ArrayAdapter themeAdapter =
        new ArrayAdapter(
            this, R.layout.spinner_simple_text, getResources().getStringArray(R.array.theme_list));
  ...
  public void onClick(View v) {
    switch (v.getId()) {
      ...
      case R.id.But_setting_theme_default:
        IV_setting_theme_main_color.setImageDrawable(
            new ColorDrawable(ColorTools.getColor(this, R.color.themeColor_custom_default)));
        IV_setting_theme_dark_color.setImageDrawable(
            new ColorDrawable(ColorTools.getColor(this, R.color.theme_dark_color_custom_default)));
      ...
      case R.id.But_setting_theme_apply:
        ...
        if (themeManager.getCurrentTheme() == ThemeManager.CUSTOM) {
          ...
          if (isAddNewProfileBg) {
            ...
            if (!"".equals(profileBgFileName)) {
              try {
                ...
                FileManager.copy(
                    new File(tempFileManager.getDirAbsolutePath() + "/" + profileBgFileName),
                    new File(
                        settingFM.getDirAbsolutePath()
                            + "/"
          ...
          SPFManager.setMainColor(
              this, ((ColorDrawable) IV_setting_theme_main_color.getDrawable()).getColor());
          SPFManager.setSecondaryColor(
              this, ((ColorDrawable) IV_setting_theme_dark_color.getDrawable()).getColor());
        }
      ...
      case R.id.IV_setting_theme_main_color:
        SettingColorPickerFragment mainColorPickerFragment =
            SettingColorPickerFragment.newInstance(
                themeManager.getThemeMainColor(this), R.id.IV_setting_theme_main_color);
      ...
      case R.id.IV_setting_theme_dark_color:
        SettingColorPickerFragment secColorPickerFragment =
            SettingColorPickerFragment.newInstance(
                themeManager.getThemeDarkColor(this), R.id.IV_setting_theme_dark_color);
                            ...
                            new Runnable() {
                              public void run() {
                                Toast.makeText(
                                        ...
                                        getString(R.string.toast_setting_successful),
                                        Toast.LENGTH_LONG)
                                    .show();
                              }
                            });
                        ...
                        SettingActivity.this.runOnUiThread(
                            new Runnable() {
                              public void run() {
                                Toast.makeText(
                                        SettingActivity.this,
                                        getString(R.string.toast_setting_wont_fix),
                                        Toast.LENGTH_LONG)
                                    .show();
                              }
                            });
                      ...
                      SettingActivity.this.runOnUiThread(
                          new Runnable() {
                            public void run() {
                              Toast.makeText(
                                      SettingActivity.this,
                                      getString(R.string.toast_setting_fail),
                                      Toast.LENGTH_LONG)
                                  .show();
                            }
                          });
